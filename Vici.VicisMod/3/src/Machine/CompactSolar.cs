...
public abstract class CompactSolar : SolarPanel {
    ...
    public CompactSolar(ModCreateSegmentEntityParameters parameters) :
        ...
        this.mrTransferRate = float.MaxValue; // POWER OF THE SUN
        this.multiplier = 1;
        lastScan = 0;
    ...
    public override void LowFrequencyUpdate() {
        ...
        float newGenerated = this.mrLastPowerGain * LowFrequencyThread.mrPreviousUpdateTimeStep;
        ...
        // Add it together
        if(newGenerated > 0) {
            this.mrLastPowerGain += newGenerated / LowFrequencyThread.mrPreviousUpdateTimeStep;
            this.mrCurrentPower += newGenerated;
    ...
    protected void findMk4AndMk5Bats() {
        ...
        for (int i = 0; i < 3; ++i) {
            for(int j = -1; j <= 1; j+=2) {
                ...
                long x = this.mnX + coords[0];
                long y = this.mnY + coords[1];
                long z = this.mnZ + coords[2];
    ...
    public override void UnityUpdate() {
        ...
        if (!this.mbLinkedToGo) {
            ...
            CPH_SolarPanel[] componentsInChildren = this.mWrapper.mGameObjectList[0].gameObject.GetComponentsInChildren<CPH_SolarPanel>();
            this.maPanels = new GameObject[componentsInChildren.Length];
            for (int i = 0; i < componentsInChildren.Length; i++) {
                this.maPanels[i] = componentsInChildren[i].gameObject;
